...
exports.createProfile = async (req, res) => {
	...
	console.log(req.body);	
        ...
        if (!firstName || !lastName || !email || !location  ||  !preference || !business_category) {
    ...
    try {
       ...
       // const user = await userModel.findOne({ userId: findUser }).select("firstName lastName bio email companyName address business_category profilePicture location userId _id");
          const user = await userModel
            .findOne({ userId: findUser })
            .select("-_id firstName lastName bio email companyName address business_category profilePicture location userId")
            .populate({
                path: "userId",
                select: " -_id phoneNumber",
            });
	const demand = await demandModel.find({ userId: findUser,Expiry:{$gte:istDateTime.date} }).select("imageUrl1 imageUrl2 imageUrl3 category shape Type size clarity color Cut _id")
...
exports.getNotifications = async (req, res) => {
  try {
    ...
    // Fetch notifications and sort by createdAt (newest first)
    let notifications = await notificationModel
      .find({ userId: userId })
      .sort({ createdAt: -1 }) // Sort by createdAt in descending order
      .lean();

    const demandIds = notifications.map((item) => item.demandId);

    // Fetch corresponding demands
    let demandData = await demandModel
      .find({ _id: { $in: demandIds } })
      .lean()
      .select("_id shape size clarity color Cut createdAt");
    ...
    // Map demands to their notifications
    const result = demandData.map((demand) => {
      const notification = notifications.find(
        (notification) => notification.demandId.toString() === demand._id.toString()
      );
      if (notification && notification.profilePic) {
        return {
          ...demand,
          profilePic: notification.profilePic,
          notificationCreatedAt: notification.createdAt, // Include createdAt for sorting
        };
      }
      return { ...demand, notificationCreatedAt: notification?.createdAt };
    });

    // Sort the result by notificationCreatedAt in descending order
    result.sort((a, b) => new Date(b.notificationCreatedAt) - new Date(a.notificationCreatedAt));
...
exports.getNotifications = async (req, res) => {
  ...
  //  const today = new Date();
   // today.setHours(0, 0, 0, 0);
    ...
    // Fetch corresponding demands that are not expired
    let demandData = await demandModel
      .find({ 
        _id: { $in: demandIds }, 
        Expiry: { $gte: istDateTime.date } // Filter demands with Expiry date >= today
      })
      .lean()
      .select("_id shape size clarity color Cut createdAt Expiry");

    if (!demandData || demandData.length === 0) {
      return res.status(404).json({
        message: "No valid demands found",
        success: false,
      });
    }
