...
exports.createDemand = async (req, res) => {
   // try {
	console.log("Hit to  createdemand route")
...
exports.getAllNaturalDemands = async (req, res) => {
	...
	const istDateTime = currentDate();
        console.log(" Date ",istDateTime.date);
	const userId = req.user;
        if (!userId) return res.status(400).json({ message: "User not exists", success: false })
            const updatedDemand = await demandModel.updateMany(
                { Expiry: { $gt: istDateTime.date }, Type: "Natural" },
                { status: "Active" },
                { new: true }
            );
            const updatedDemandin = await demandModel.updateMany(
                { Expiry: { $lt: istDateTime.date }, Type: "Natural" },
                { status: "Inactive" },
                { new: true }
            ); 
        const allDemands = await demandModel.find({ userId: { $ne: userId }, Type: "Natural", Expiry: { $gte: istDateTime.date } }).sort({ fetchTime: -1 });
...
exports.getAllLabDemands = async (req, res) => {
	...
	const istDateTime = currentDate();
        ...
        const allDemands = await demandModel.find({ userId: { $ne: userId }, Type: "Lab Grown", Expiry: { $gte: istDateTime.date } }).sort({ fetchTime: -1 });
        return res.status(200).json({ demands: allDemands, success: true });
    } catch (error) {
        return res.status(500).json({ message: "Failed to fetch demands", success: false });
    }
...
exports.getNaturalSpecificDemands = async (req, res) => {
	...
	const istDateTime = currentDate();
        ...
        const demands = await demandModel.find({ userId: { $ne: userId }, category: category, Type: "Natural", Expiry: { $gte: istDateTime.date } }).sort({ fetchTime: -1 });
...
exports.getLabSpecificDemands = async (req, res) => {
	...
	const userId = req.user;
        ...
        const demands = await demandModel.find({ userId: { $ne: userId }, category: category, Type: "Lab Grown", Expiry: { $gte: istDateTime.date } }).sort({ fetchTime: -1 });
...
exports.myNaturalDemands = async (req, res) => {
	...
	const istDateTime = currentDate();
        ...
        const updatedDemand = await demandModel.updateMany(
            { Expiry: { $gte: istDateTime.date }, userId: userId, Type: "Natural" },
            { status: "Active" },
            { new: true }
        );
        console.log(updatedDemand)
        const updatedDemandin = await demandModel.updateMany(
            { Expiry: { $lt: istDateTime.date }, userId: userId, Type: "Natural" },
            { status: "Inactive" },
            { new: true }
        );
...
exports.myLabDemands = async (req, res) => {
	...
	const userId = req.user;
        ...
        const updatedDemand = await demandModel.updateMany(
            { Expiry: { $gte: istDateTime.date }, userId: userId, Type: "Lab Grown" },
            { status: "Active" }
        );

        const updatedDemandin = await demandModel.updateMany(
            { Expiry: { $lt: istDateTime.date }, userId: userId, Type: "Lab Grown" },
            { status: "Inactive" }
        );
...
exports.myNaturalActiveDemands = async (req, res) => {
	...
	const userId = req.user;
        ...
        console.log(updatedDemand)
        const myDemands = await demandModel.find({ userId: userId, Type: "Natural", Expiry: { $gte: istDateTime.date } }).sort({ fetchTime: -1 }).select("imageUrl shape Type size clarity color cut fetchTime userId _id status");
...
exports.myNaturalInactiveDemands = async (req, res) => {
	...
	const userId = req.user;
        ...
        const myDemands = await demandModel.find({ userId: userId, Type: "Natural", Expiry: { $lt: istDateTime.date } }).sort({ fetchTime: -1 }).select("imageUrl1 imageUrl2 imageUrl3 shape Type size clarity fetchTime color cut status userId _id");
        return res.status(200).json({ myDemands: myDemands, success: true });
    } catch (error) {
        return res.status(500).json({ message: "Failed to fetch demand", success: false });
...
exports.myLabActiveDemands = async (req, res) => {
	...
	const userId = req.user;
        ...
        console.log(updatedDemand)
        const myDemands = await demandModel.find({ userId: userId, Type: "Lab Grown", Expiry: { $gt: istDateTime.date } }).sort({ fetchTime: -1 }).select("imageUrl shape Type size clarity color cut fetchTime status userId _id");
        return res.status(200).json({ myDemands: myDemands, success: true });
    } catch (error) {
        return res.status(500).json({ message: "Failed to fetch demand", success: false });
...
exports.myLabInactiveDemands = async (req, res) => {
	...
	const istDateTime = currentDate();
        ...
        console.log(updatedDemand)
        const myDemands = await demandModel.find({ userId: userId, Type: "Lab Grown", Expiry: { $lt: istDateTime.date } }).sort({ fetchTime: -1 }).select("imageUrl shape Type size clarity color cut fetchTime status userId _id ");
        return res.status(200).json({ myDemands: myDemands, success: true });
    } catch (error) {
        return res.status(500).json({ message: "Failed to fetch demand", success: false });
